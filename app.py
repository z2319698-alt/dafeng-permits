...
try:
    ...
    # 7. 第三層按鈕判定
    acts = []
    if "清除" in raw_type: acts = ["變更", "變更暨展延", "展延"]
    elif "清理" in raw_type: acts = ["變更", "展延", "異動"]
    elif "水污染" in raw_type: acts = ["事前變更", "事後變更", "展延"]
    else: acts = ["展延"]
    ...
    if st.session_state.get("last_p") == sel_n and "cur_a" in st.session_state:
        ...
        with st.expander("⚖️ 第一步：法規依據條件確認", expanded=True):
            m_key = next((k for k in LAW_REQUIREMENTS if k in raw_type), None)
            conds = LAW_REQUIREMENTS[m_key].get(curr_act, ["參考縣市自主檢查表"]) if m_key else ["參考規範"]
            sel_laws = [c for c in conds if st.checkbox(c, key=f"l_{sel_n}_{curr_act}_{c}")]
            ...
            if u_name:
                ...
                if attach_db is not None:
                    ...
                    for _, r in attach_db.iterrows():
                        # 匹配類型關鍵字(前兩個字)與項目關鍵字(前兩個字)
                        if str(sel_t)[:2] in str(r.iloc[0]) and str(curr_act)[:2] in str(r.iloc[1]):
                            if pd.notnull(r.iloc[2]):
                                final_items.append(str(r.iloc[2]).strip())
                
                # 去除重複
                final_items = list(dict.fromkeys(final_items))
                ...
                if final_items:
                    ...
                    if st.button("🚀 提出申請並發信", use_container_width=True):
                        ...
                        st.markdown(f'<a href="{mailto_url}" style="background-color:#4CAF50;color:white;padding:12px;text-decoration:none;border-radius:5px;display:block;text-align:center;">📧 按此啟動郵件系統發信</a>', unsafe_allow_html=True)
                else:
                    st.warning("⚠️ 查無附件清單，請檢查 Excel 分頁內容。")
            else:
