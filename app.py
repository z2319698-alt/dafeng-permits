...
# --- 引用根目錄零件 (請確保 ai_engine.py 和 ui_components.py 已在根目錄) ---
try:
    from ai_engine import ai_verify_background
    from ui_components import display_penalty_cases
except ImportError:
    ...
    if st.session_state.mode == "home":
            ...
            if pd.notna(url) and str(url).strip().startswith("http"):
                ...
                if not is_match:
                    ...
                    with st.expander(f"🛠️ 修正 {p_name}"):
                        ...
                        new_date = col_fix.date_input("正確到期日", value=p_date.date() if pd.notnull(p_date) else date.today(), key=f"fix_{idx}")
                        if col_fix.button("確認修正", key=f"btn_fix_{idx}", type="primary", use_container_width=True):
        ...
        if options:
            ...
            for i, opt in enumerate(options):
                if action_cols[i].button(opt, key=f"act_{opt}", use_container_width=True, type="primary" if opt in st.session_state.selected_actions else "secondary"):
