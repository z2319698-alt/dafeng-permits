...
def display_ai_law_wall(category):
    law_db = {
        "廢棄物清理計畫書": [
            {"date": "2025/08", "tag": "重大變更", "content": "環境部公告：廢清書應增列「資源循環促進」專章，路徑須明確揭露。"},
            {"date": "2025/11", "tag": "裁罰預警", "content": "產源端若未落實現場視察，發生違法傾倒時將面臨連帶重罰。"},
            {"date": "2026/01", "tag": "最新公告", "content": "全面推動電子化合約上傳，紙本備查期縮短至14天。"}
        ],
        "水污染防治許可證": [
            {"date": "2025/07", "tag": "標準加嚴", "content": "氨氮、重金屬指標納入年度評鑑，監測異常將凍結展延權限。"}
        ]
    }
    updates = law_db.get(category, [{"date": "2025-2026", "tag": "穩定", "content": "法規動態穩定，維持定期申報即可。"}])
...
def display_penalty_cases():
    ...
    cases = [
        {"type": "2025/09 廢棄物非法棄置刑案", "reason": "屏東包商未領許可私自收受廢棄物並傾倒，涉及刑事責任。", "key": "務必核對廠商證號與流向證明。"},
        {"type": "2026/02 農地盜採回填案", "reason": "回填14萬噸廢棄物於水源區，不法獲利2.4億。", "key": "產源單位若無法證明流向將負擔高額清理費。"}
    ]
    for case in cases:
    ...
    news = [
        {"topic": "南投名間焚化爐修繕抗爭", "desc": "焚化爐老舊維修導致收受縮減，引發地方抗爭。", "advice": "加強場內分類。"},
        {"topic": "環境部科技監控專案啟動", "desc": "擴大採用AI影像與GPS比對，軌跡異常將自動稽查。", "advice": "按申報路線行駛。"},
        {"topic": "社群媒體即時爆料檢舉", "desc": "民眾針對異味投訴轉向Dcard/FB，引發局端查訪。", "advice": "落實日常巡檢。"},
        {"topic": "許可申報代碼誤植稽查", "desc": "重點針對營建廢棄物與事廢代碼混用情形。", "advice": "執行代碼複核。"}
    ]
...
try:
    ...
    if st.sidebar.button("🏠 系統首頁"): 
        st.session_state.mode = "home"
        st.rerun()
    if st.sidebar.button("📋 許可證辦理系統"): 
        st.session_state.mode = "management"
        st.rerun()
    if st.sidebar.button("📁 許可下載區"): 
