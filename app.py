...
try:
    ...
    if st.session_state.mode == "library":
        st.header("📁 既有文件下載區")
        # 修正：加上 enumerate 產出唯一 index 作為按鈕 key
        for idx, row in main_df.iterrows():
            ...
            if pd.notna(url) and str(url).startswith("http"):
                # 這裡加上 key=f"dl_{idx}" 解決重複 ID 錯誤
                c3.link_button("📥 下載 PDF", str(url), use_container_width=True, key=f"dl_{idx}")
            else:
                c3.button("❌ 無連結", disabled=True, use_container_width=True, key=f"no_dl_{idx}")
            st.divider()

    elif st.session_state.mode == "management":
        # (保留原本所有選擇器、AI感測器與辦理按鈕)
        ...
        if options:
            ...
            for i, option in enumerate(options):
                ...
                if cols[i].button(option, key=f"btn_act_{option}", use_container_width=True, type="primary" if is_active else "secondary"):
            ...
            if st.session_state.selected_actions:
                ...
                if st.button("🚀 提出申請", type="primary", key="final_submit_btn"):
                    if user_name:
