...
def display_penalty_cases():
    ...
    all_news = [
        ...
        {"topic": "申報代碼誤植連罰", "desc": "稽查熱點：夾帶營建廢材認定申報不實。", "advice": "定期執行代碼複核。"}
    ]
...
try:
    ...
    if st.session_state.mode == "library":
        ...
        if days_left < 90:
            ...
            st.markdown(f'<div style="background-color: #ffeded; padding: 15px; border-radius: 10px; color: #333;"><b>🤖 AI 建議：</b>立即準備附件申報，避免罰鍰。</div>', unsafe_allow_html=True)
        elif days_left < 180:
            st.warning(f"⚠️ 【到期預警】剩餘 {days_left} 天。")
            st.markdown(f'<div style="background-color: #fff9e6; padding: 15px; border-radius: 10px; color: #333;"><b>🤖 AI 建議：</b>建議開始核對附件，預留審補時間。</div>', unsafe_allow_html=True)
        else:
            st.success(f"✅ 【狀態正常】剩餘 {days_left} 天。")
            st.markdown(f'<div style="background-color: #e8f5e9; padding: 15px; border-radius: 10px; color: #333;"><b>🤖 AI 建議：</b>在 180 天前開始蒐集即可。</div>', unsafe_allow_html=True)
        ...
        if options:
            ...
            if st.session_state.selected_actions:
                ...
                for action in st.session_state.selected_actions:
                    ...
                    if not rows.empty:
                        for item in rows.iloc[0, 3:].dropna().tolist(): final_atts.add(str(item).strip())
